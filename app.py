...
# ====================== MINIO SETUP (using your exact Railway env vars) ======================
...
def trim_video():
    ...
    file = request.files['file']
    start = request.form.get('start', 0, type=float)
    duration = request.form.get('duration', 60, type=float)
...
# ====================== BURN CAPTIONS ======================
@app.route('/v1/video/caption', methods=['POST'])
def caption_video():
    if 'file' not in request.files:
        return jsonify({"error": "No file uploaded"}), 400
    file = request.files['file']
